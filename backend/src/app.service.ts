...
import { Color, RacePositions, Racer, RacerPosition, Winner } from './racerTypes';
...
export class AppService {
  ...
  public race(): Observable<{
    ...
    return interval(1000).pipe(
        ...
        takeWhile((value) => {
          return value.racerPosition.filter((p) => p.position >= 105).length === 0
        })
    );
  }
